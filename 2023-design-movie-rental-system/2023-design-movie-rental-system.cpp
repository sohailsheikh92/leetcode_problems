...
private:
    set<pair<int, pair<int, int>>> s; // (price, (shop, movie)) - available
    unordered_map<long long, int> mp; // (shop*1e6 + movie) -> price
    set<pair<int, pair<int, int>>> rented; // rented movies
    unordered_map<int, set<pair<int, int>>> movieToPS; // movie -> (price, shop)
